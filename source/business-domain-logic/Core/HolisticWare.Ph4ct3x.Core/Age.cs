...
namespace HolisticWare.Ph4ct3x
{
    ...
    /// </summary>
    /// <see cref="https://gist.github.com/faisalman"/>
    /// <see cref="https://gist.github.com/faisalman/1724253"/>
    public partial class Age
    {
        ...
        public Age(DateTime date_of_birth)
        {
            this.Count(date_of_birth);
        }

        public Age(DateTime date_of_birth, DateTime date_current)
        {
            this.Count(date_of_birth, date_current);
        }

        public Age Count(DateTime date_of_birth)
        {
            return this.Count(date_of_birth, DateTime.Today);
        }

        public Age Count(DateTime date_of_birth, DateTime date_current)
        {
            if 
                (
                ...
                )
            {
                ...
                if (date_current.Month > date_of_birth.Month)
                {
                    ...
                    this.Months = date_current.Month - (date_of_birth.Month + 1) + Math.Abs(DaysRemain / days_in_dob);
                ...
                else if (date_current.Month == date_of_birth.Month)
                {
                    ...
                    else
                    {
                        ...
                        this.Months = 11;
                        this.Days = DateTime.DaysInMonth(date_of_birth.Year, date_of_birth.Month) - (date_of_birth.Day - date_current.Day);
                    }
                }
                else
                {
                    this.Years = (date_current.Year - 1) - date_of_birth.Year;
                    this.Months = date_current.Month + (11 - date_of_birth.Month) + Math.Abs(DaysRemain / days_in_dob);
                    this.Days = (DaysRemain % days_in_dob + days_in_dob) % days_in_dob;
                }
            }
            else
